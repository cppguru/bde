...
// On these platforms `TimeUtil::getThread*` is expected to return non-zero
// per-thread CPU values.  Elsewhere (e.g. AIX, HP-UX, Cygwin) the
// implementation documents that it may return -1, in which case the
// "thread-timer increases" assertions in this driver are skipped.
#if defined(BSLS_PLATFORM_OS_LINUX)   || defined(BSLS_PLATFORM_OS_FREEBSD)  \
 || defined(BSLS_PLATFORM_OS_DARWIN)  || defined(BSLS_PLATFORM_OS_WINDOWS)  \
 || defined(BSLS_PLATFORM_OS_SOLARIS)
    #define U_BSLS_TIMEUTIL_HAS_THREAD_TIMERS 1
#endif
...
// [ 1] Int64 getThreadSystemTimer();
// [ 1] void getThreadTimers(Int64*, Int64*);
// [ 1] Int64 getThreadUserTimer();
...
static Int64 callGetThreadTimersRetSystem() {
    Int64 cTimer;
    Int64 uTimer;
    TU::getThreadTimers(&cTimer, &uTimer);
    return cTimer;
}

static Int64 callGetThreadTimersRetUser() {
    Int64 cTimer;
    Int64 uTimer;
    TU::getThreadTimers(&cTimer, &uTimer);
    return uTimer;
...
/// This class implements a simple interval timer that is created in the
/// "running" state, and may be queried for its cumulative interval (as a
/// `double`, in seconds) but never stopped or reset.
class my_Timer {
  private:
    // DATA
  ...
  public:
    ...
    /// Create a timer object initialized with the times at creation.  All
    /// values returned by subsequent calls to `elapsed<...>Time()` are with
    /// respect to this creation time.
    my_Timer()
    {
    ...
    /// Return the total elapsed time in seconds since the creation of this
    /// timer object.
    double elapsedWallTime();

    /// Return the elapsed user time in seconds since the creation of this
    /// timer object.
    double elapsedUserTime();

    /// Return the elapsed system time in seconds since the creation of this
    /// timer object.
...
/// base their calculations on different initial times, their results may
/// differ by at most 1 due to rounding error.
                                     ...
                                     const bsls::Types::Int64&   frequency)
{
    ...
    const bsls::Types::Int64 realStart = TU::convertRawTime(startTime);
    const bsls::Types::Int64 fakeStart = fakeConvertRawTime(startTime.d_opaque,
                                                            startTime.d_opaque,
                                                            frequency);
    const bsls::Types::Int64 realEnd   = TU::convertRawTime(endTime);
    const bsls::Types::Int64 fakeEnd   = fakeConvertRawTime(endTime.d_opaque,
                                                            startTime.d_opaque,
                                                            frequency);
...
int main(int argc, char *argv[])
{
    ...
    switch (test) { case 0:  // Zero is always the leading case.
      case 10: {
        ...
        //   The usage examples provided in the component header must build and
        //   run with a normal exit status.
        //
        // Plan:
        //   Copy the implementation portions of the Usage Examples to the
        ...
        {
            Int64 threadSystemStart;
            Int64 threadUserStart;
            TU::getThreadTimers(&threadSystemStart, &threadUserStart);
            ...
            if (threadSystemStart >= 0 && threadUserStart >= 0) {
                ...
                Int64 threadSystemEnd;
                Int64 threadUserEnd;
                TU::getThreadTimers(&threadSystemEnd, &threadUserEnd);
                ...
                ASSERTV(threadSystemStart,
                        threadSystemEnd,
                        threadSystemStart <= threadSystemEnd);
                ASSERTV(threadUserStart,
                        threadUserEnd,
                        threadUserStart <= threadUserEnd);
                ...
                ASSERTV(threadSystemStart, -1 == threadSystemStart);
                ASSERTV(threadUserStart,   -1 == threadUserStart);

                if (verbose) {
                    puts("thread CPU timers are not supported on this "
                         "platform");
                }
            }
        }
      ...
      case 9: {
        ...
        {
            if (verbose)
                puts("\nCompare fakeConvertRawTime to convertRawTime with "
                     "static data"
                     "\n--------------------------------------------------"
                     "-----------");
...

        {
            ...
            for (int bits = 0;
                ...
                for (int iterations = 0;
                    ...
                    if (veryVerbose) {
                        T_ P_(bits) P_(iterations); (offset);
                    }
      ...
      case 6: {
        // --------------------------------------------------------------------
        // PERFORMANCE TEST `gethrtime()` *** Sun ONLY ***
        ...
        //
        // Testing:
        //   Test of `gethrtime()` (Sun only -- statistical)
        // --------------------------------------------------------------------
      ...
      case 5: {
        ...
        //   rely on the fact that subsequent calls to either `times` or the
        ...
        //
        //   5. On platforms with per-thread CPU timers, values returned by
        //      `getThreadSystemTimer`, `getThreadUserTimer`, and
        //      `getThreadTimers` are consistent with each other and with
        //      process CPU timers in this single-threaded test driver.
        //
        //   6. On platforms without per-thread CPU timers, thread timer
        //      methods return -1.
        //
        // Plan:
        ...
        //   2. For thread timers:
        //
        //      1. On supported platforms, burn CPU time and verify thread
        //         timer values are non-decreasing, accessor and pair-return
        //         methods are consistent, and thread CPU usage does not exceed
        //         process CPU usage (within quantization).
        //
        //      2. On unsupported platforms, verify thread timer accessors and
        //         pair-return method return -1.
        //
        // Testing:
        //   Forwarding of methods to underlying OS APIs (Unix only),
        //   including thread CPU-time APIs
        // --------------------------------------------------------------------
...

        {
            ...
            if (veryVeryVerbose) {
                if (elapsedSystemTime) {
                    ...
                    puts("no elapsed system time");
                }
            }
        }

        if (verbose) {
            puts("\nTesting thread CPU timers.\n");
        }

        #ifdef U_BSLS_TIMEUTIL_HAS_THREAD_TIMERS
        {
            const int NUM_THREAD_INTERVALS = 5;

            Int64 previousThreadSystem = TU::getThreadSystemTimer();
            Int64 previousThreadUser   = TU::getThreadUserTimer();

            ASSERTV(previousThreadSystem, previousThreadSystem >= 0);
            ASSERTV(previousThreadUser,   previousThreadUser   >= 0);

            for (int i = 0; i < NUM_THREAD_INTERVALS; ++i) {
                int u = 0;
                burnUserTime(&u);

                Int64 processSystem = TU::getProcessSystemTimer();
                Int64 processUser   = TU::getProcessUserTimer();

                Int64 threadSystem;
                Int64 threadUser;
                TU::getThreadTimers(&threadSystem, &threadUser);

                Int64 threadSystemSingle = TU::getThreadSystemTimer();
                Int64 threadUserSingle   = TU::getThreadUserTimer();

                ASSERTV(i, previousThreadSystem, threadSystem,
                        previousThreadSystem <= threadSystem);
                ASSERTV(i, previousThreadUser, threadUser,
                        previousThreadUser <= threadUser);

                ASSERTV(i, threadSystem, threadSystemSingle,
                        threadSystem <= threadSystemSingle +
                                                      MODEL_CPU_TICK_DURATION);
                ASSERTV(i, threadUser, threadUserSingle,
                        threadUser <= threadUserSingle +
                                                      MODEL_CPU_TICK_DURATION);

                ASSERTV(i, threadSystem, processSystem,
                        threadSystem <= processSystem +
                                                  2 * MODEL_CPU_TICK_DURATION);
                ASSERTV(i, threadUser, processUser,
                        threadUser <= processUser +
                                                  2 * MODEL_CPU_TICK_DURATION);

                previousThreadSystem = threadSystemSingle;
                previousThreadUser   = threadUserSingle;
            }
        }
        #else  // U_BSLS_TIMEUTIL_HAS_THREAD_TIMERS
        {
            Int64 threadSystem;
            Int64 threadUser;
            TU::getThreadTimers(&threadSystem, &threadUser);

            ASSERTV(threadSystem, -1 == threadSystem);
            ASSERTV(threadUser,   -1 == threadUser);

            const Int64 threadSystemSingle = TU::getThreadSystemTimer();
            const Int64 threadUserSingle   = TU::getThreadUserTimer();

            ASSERTV(threadSystemSingle, -1 == threadSystemSingle);
            ASSERTV(threadUserSingle,   -1 == threadUserSingle);
        }
        #endif  // else - U_BSLS_TIMEUTIL_HAS_THREAD_TIMERS
#endif  // UNIX but not 32-bit Linux
      } break;
      case 4: {
        // --------------------------------------------------------------------
        // HOOKING TEST
        ...
        //
        //   That the thread CPU-time calls are consistent with process
        //   CPU-time calls on platforms with per-thread CPU timers, and
        //   return -1 on unsupported platforms.
        //
        // Plan:
        ...
        //
        //   4. Burn CPU time in this (single) thread and compare process
        //      versus thread CPU timers.  Verify thread timers increase and
        //      do not exceed process timers (within a quantization margin) on
        //      supported platforms.
        //
        //   5. On unsupported platforms, verify thread timer values satisfy
        //      basic monotonicity invariants; exact values are platform-
        //      dependent and may be -1.
        //
        // Testing:
        //   Forwarding of methods to underlying OS APIs
        //   including thread-time APIs
        // --------------------------------------------------------------------
...
# error Need platform info to find time quantum!
        ...
        // Third subtest: Consume time; verify that it appears in the counts.
        {
            ...
            const Int64 wt1 = TU::getTimer();
            const Int64 ut1 = TU::getProcessUserTimer();
            const Int64 st1 = TU::getProcessSystemTimer();
            ...
            const Int64 wt2 = TU::getTimer();
            const Int64 ut2 = TU::getProcessUserTimer();
            const Int64 st2 = TU::getProcessSystemTimer();
        ...
        // Fourth subtest: thread CPU times.  Because this test driver is
        // single-threaded, the thread CPU times should track the process CPU
        // times (modulo measurement noise) on platforms that report them.
        {
            if (verbose)
                puts("\nHooking Test: Thread CPU time over long loop."
                     "\n=============================================");

            const unsigned longLoop = 50 * 1000L * 1000L;

            const Int64 put1 = TU::getProcessUserTimer();
            const Int64 pst1 = TU::getProcessSystemTimer();
            const Int64 tut1 = TU::getThreadUserTimer();
            const Int64 tst1 = TU::getThreadSystemTimer();
            ...
            const Int64 put2 = TU::getProcessUserTimer();
            const Int64 pst2 = TU::getProcessSystemTimer();
            const Int64 tut2 = TU::getThreadUserTimer();
            const Int64 tst2 = TU::getThreadSystemTimer();

            if (veryVerbose) {
                printf("put: %lld -> %lld = %lld\n", put1, put2, put2 - put1);
                printf("pst: %lld -> %lld = %lld\n", pst1, pst2, pst2 - pst1);
                printf("tut: %lld -> %lld = %lld\n", tut1, tut2, tut2 - tut1);
                printf("tst: %lld -> %lld = %lld\n", tst1, tst2, tst2 - tst1);
            }

            // Time never goes backward.
            ASSERTV(tut1, tut2, tut2 - tut1 >= 0);
            ASSERTV(tst1, tst2, tst2 - tst1 >= 0);

#if defined(U_BSLS_TIMEUTIL_HAS_THREAD_TIMERS)
            // On platforms that expose per-thread CPU times, the burning
            // loop spent at least 'timeQuantum' worth of user CPU time on
            // this very thread.
            ASSERTV(tut2 - tut1, timeQuantum, tut2 - tut1 >= timeQuantum);

            // The current thread cannot have consumed more CPU time than the
            // whole process did over the same interval, modulo a quantum of
            // measurement noise on each end.
            ASSERTV(tut2 - tut1, put2 - put1, timeQuantum,
                    (tut2 - tut1) <= (put2 - put1) + 2 * timeQuantum);
            ASSERTV(tst2 - tst1, pst2 - pst1, timeQuantum,
                    (tst2 - tst1) <= (pst2 - pst1) + 2 * timeQuantum);
#else
            // The implementation will return -1 on unsupported platforms.
            (void)put1; (void)put2; (void)pst1; (void)pst2;
#endif
        }
      } break;
      case 3: {
      ...
      case 2: {
        ...
        TimerMethods[] = {
            ...
            { TU::getThreadSystemTimer,      "getThreadSystemTimer"     },
            { TU::getThreadUserTimer,        "getThreadUserTimer"       },
            { callGetThreadTimersRetSystem,  "getThreadTimers(system)"  },
            { callGetThreadTimersRetUser,    "getThreadTimers(user)"    }
        };
        size_t const TimerMethodsCount =
                                  sizeof TimerMethods / sizeof TimerMethods[0];
      ...
      case 1: {
        ...
        //                                        bsls::Types::Int64);
        //   bsls::Types::Int64 bsls::TimeUtil::getThreadSystemTimer();
        //   bsls::Types::Int64 bsls::TimeUtil::getThreadUserTimer();
        //   void bsls::TimeUtil::getThreadTimers(bsls::Types::Int64*,
        //                                       bsls::Types::Int64*);
        // --------------------------------------------------------------------
...

        {
            ...
            for (int i = 0; i < NUM_SCENARIOS; ++i) {
                ...
                for (int j = 1; j < NUM_SAMPLES; j++) {
                    ...
                    // high-resolution measurement for the resource we are
                    // targeting.
